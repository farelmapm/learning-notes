...
- `id` - What group does the current user belong to
- `hostname` what is the server named
- `ifconfig`/`ip a` - network details
- `sudo -l` - can current user run sudo

### OS version
```
farelmusyaffa@htb[/htb]$ cat /etc/os-release
...
### Shells
```
farelmusyaffa@htb[/htb]$ cat /etc/shells
...
```
Try to mount additional or unmounted drives/file systems

### Printers
`lpstat` - Check for active or queued print jobs for sensitive informations
...
```
Search for credentials in `fstab` for mounted drives by `grep`-ing common words such as `password`, `username`, `credential`

### Routing table
```
farelmusyaffa@htb[/htb]$ route
...
### resolv.conf
Check `/etc/resolv.conf` if the host is configured to use internal DNS to query for Active Directory environment

### ARP table
```
farelmusyaffa@htb[/htb]$ arp -a

_gateway (10.129.0.1) at 00:50:56:b9:b9:fc [ether] on ens192
```
Check what other hosts the target has been communicating with

### Password Hash Blocks
...
```
farelmusyaffa@htb[/htb]$ grep "sh$" /etc/passwd
...
```
Vulnerable versions such as Bash version 4.1 might be vulnerable to `shellshock` exploit

### User groups
```
farelmusyaffa@htb[/htb]$ cat /etc/group
...
```
farelmusyaffa@htb[/htb]$ getent group sudo

sudo:x:27:mrb3n
```

### Things to Check
- `.bash_history` - Interesting commands, config files
- SSH keys
- ARP cache

### Mounted File Systems
```
farelmusyaffa@htb[/htb]$ df -h
...
```
farelmusyaffa@htb[/htb]$ cat /etc/fstab | grep -v "#" | column -t
...
### All Hidden Files
```
farelmusyaffa@htb[/htb]$ find / -type f -name ".*" -exec ls -l {} \; 2>/dev/null | grep htb-student
...
```
farelmusyaffa@htb[/htb]$ find / -type d -name ".*" -ls 2>/dev/null
...
### Temporary Files
```
farelmusyaffa@htb[/htb]$ ls -l /tmp /var/tmp /dev/shm
...
`/tmp` and `/var/tmp` are used to store data temporarily
- `/var/tmp` retained for up to 30 days and stays after reboot
- `/tmp` retained for up to 10 days and gets deleted after reboot
